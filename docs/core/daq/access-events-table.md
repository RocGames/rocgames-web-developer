...
| 隐私协议展示事件 | privacy_policy_show         | 隐私协议展示时触发                 |     -      |    -     |     -    |     -     |   -    |   -   |
|隐私协议已同意事件| privacy_policy_agree        | 隐私协议同意后触发                   |     -      |    -     |     -    |     -     |   -    |   -   |
...
| 玩家充值货币增加 | earn_currency | 用户成功充值现金货币时触发 | uid | serverid | playerid | type（type（货币类型:如钻石、代金劵）） | value（数值） | amount（流动后账户总额度, 通常为游戏币） |- |
| 玩家充值货币消耗 | spend_currency | 用户成功消费现金货币时触发 | uid | serverid | playerid | type（type（货币类型:如钻石、代金劵）） | value（数值） | amount（流动后账户总额度, 通常为游戏币） |- |
