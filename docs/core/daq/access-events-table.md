...
| 玩家充值 | earn_currency | 用户成功充值现金货币时触发 | uid | serverid | playerid | type（type（货币类型:如钻石、代金劵）） | value（数值） | amount（流动后账户总额度, 通常为游戏币） |- |
| 玩家充值 | spend_currency | 用户成功消费现金货币时触发 | uid | serverid | playerid | type（type（货币类型:如钻石、代金劵）） | value（数值） | amount（流动后账户总额度, 通常为游戏币） |- |
